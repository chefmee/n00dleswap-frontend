    ...
    <WindowContent>
      {nfts.length !== 1 ? (
        <>
          Filter:{" "}
          <TextField
            placeholder="keyword or NFT contract address"
      ...
      {formErrors.startPrice.length > 0 && (
        <p style={{ color: "red" }}>{formErrors.startPrice}</p>
      )}
      <p>
        Start Price (ETH):{" "}
        <TextField
          onChange={(e) => onFieldChanged("startPrice", e)}
          type="number"
        ></TextField>
      </p>
      {formErrors.priceIncrement.length > 0 && (
        <p style={{ color: "red" }}>{formErrors.priceIncrement}</p>
      )}
      <p>
        Price Decrement (ETH):{" "}
        <TextField
          onChange={(e) => onFieldChanged("priceIncrement", e)}
          type="number"
        ></TextField>
      </p>
      {formErrors.stakeAmount.length > 0 && (
        <p style={{ color: "red" }}>{formErrors.stakeAmount}</p>
      )}
      <p>
        Amount to stake (ETH):{" "}
        <TextField
          onChange={(e) => onFieldChanged("stakeAmount", e)}
          type="number"
        ></TextField>
      </p>
      <p>
        The first NFT being sold to this pool will have a sell price of{" "}
        {startPrice} ETH and the second will be sold at{" "}
        {Number(startPrice) - Number(priceIncrement)} ETH, etc.
      </p>
      <p>Step 1:</p>
      <Button
        disabled={
          new BigNumber(WETHAllowance.data?.toString()).gte(
            new BigNumber(stakeAmount).times("1000000000000000000")
          ) ||
          isWETHApproveLoading ||
          !stakeAmount ||
          !startPrice
        }
        onClick={() => writeWETHApprove?.()}
      >
        {isWETHApproveLoading
          ? "Approving Factory..."
          : new BigNumber(WETHAllowance.data?.toString()).gte(
      ...
      <Button
        ...
        disabled={isCreateLoading || isCreateSuccess || !formValid}
      >
        {isCreateLoading
          ? "Making Offer..."
          : isCreateSuccess
